...
public class TobMistakeTrackerPanel extends PluginPanel {
    ...
    public TobMistakeTrackerPanel(Client client, MistakeStateReader mistakeStateReader,
        ...
        SwingUtil.removeButtonDecorations(toggleBtn);
        toggleBtn.setIconTextGap(0);
        toggleBtn.setIcon(ROOM_DEATHS_ICON);
        toggleBtn.setRolloverIcon(RAID_DEATHS_ICON_HOVER);
        toggleBtn.setSelectedIcon(RAID_DEATHS_ICON);
        toggleBtn.setRolloverSelectedIcon(ROOM_DEATHS_ICON_HOVER);
        toggleBtn.setBackground(ColorScheme.DARKER_GRAY_COLOR);
        toggleBtn.setUI(new BasicToggleButtonUI()); // substance breaks the layout and the pressed icon
        toggleBtn.addActionListener(e -> toggleDeathGrouping());
        toggleBtn.setSelected(true);
        SwingUtil.addModalTooltip(toggleBtn, "Show All deaths", "Show each death");

        // Add all action buttons to the header
        actionButtons.add(raidDeathsBtn);
        actionButtons.add(roomDeathsBtn);
        actionButtons.add(toggleBtn);
    ...
    private void toggleDeathGrouping() {
        isRaidDeaths = !isRaidDeaths;
        toggleBtn.setSelected(isRaidDeaths);
        rebuildAll();
    }
